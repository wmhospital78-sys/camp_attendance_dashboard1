...
# Settings helpers
# ---------------------------
DEFAULT_THEME = {"primary":"#131321","card":"#1e1e2f","accent":"#9b6bff","bg":"#0f0f13","text":"#EAEAEA"}
...
def render_css():
    ...
    section[data-testid="stSidebar"]{{background:{primary} !important;color:{text} !important;padding:18px;border-radius:14px;box-shadow:6px 0 30px rgba(0,0,0,0.6);}}
    .dash-card{{background:{card};padding:18px;border-radius:12px;box-shadow:0 8px 30px rgba(0,0,0,0.6);color:{text};margin-bottom:12px;}}
    .dash-card h3{{margin:0;font-size:22px;font-weight:700;}}
    .dash-card p{{margin:0;color:#bdbdbd;}}
    div.stButton>button{{background: linear-gradient(90deg,{accent},#6d3cff)!important;color:white;border-radius:10px;padding:8px 14px;font-weight:700;}}
    div.stButton>button:hover{{box-shadow:0 6px 20px rgba(107,63,255,0.25);}}
    </style>
    """
...
# ---------------------------
def fetch_staff_df():
    conn=get_conn(); df=pd.read_sql("SELECT * FROM staff ORDER BY COALESCE(serial_no,id)",conn); conn.close()
    if df.empty: return pd.DataFrame(columns=["id","serial_no","name","category","pg_year","joining_date","camps_attended"])
    return df

def insert_staff(serial_no,name,category,pg_year,joining_date,camps_attended):
    conn=get_conn(); cur=conn.cursor()
    cur.execute("INSERT INTO staff(serial_no,name,category,pg_year,joining_date,camps_attended) VALUES (?,?,?,?,?,?)",
                (serial_no,name,category,pg_year,str(joining_date),camps_attended))
    conn.commit(); conn.close()

def update_staff_row(row:dict):
    conn=get_conn(); cur=conn.cursor()
    cur.execute("UPDATE staff SET serial_no=?,name=?,category=?,pg_year=?,joining_date=?,camps_attended=? WHERE id=?",
...
def delete_staff_by_id(id_):
    conn=get_conn(); cur=conn.cursor(); cur.execute("DELETE FROM staff WHERE id=?",(id_,)); conn.commit(); conn.close()

def import_staff_from_df(df:pd.DataFrame,replace=False):
    conn=get_conn()
    if replace: conn.execute("DELETE FROM staff")
    df=df.copy(); expected=["serial_no","name","category","pg_year","joining_date","camps_attended"]
    col_map={}
    for col in df.columns:
        lc=col.strip().lower()
        if "serial" in lc or "sl" in lc: col_map[col]="serial_no"
        elif "name" in lc: col_map[col]="name"
        elif "category" in lc: col_map[col]="category"
        elif "pg" in lc or "year" in lc: col_map[col]="pg_year"
        elif "join" in lc or "doj" in lc: col_map[col]="joining_date"
        elif "camp" in lc or "attend" in lc: col_map[col]="camps_attended"
    df=df.rename(columns=col_map)
    for e in expected:
        if e not in df.columns: df[e]=None
...
# Camps helpers
def fetch_camps_df():
    conn=get_conn(); df=pd.read_sql("SELECT * FROM camps ORDER BY camp_date DESC",conn); conn.close()
    if df.empty: return pd.DataFrame(columns=["id","title","camp_date"]); return df

def insert_camp(title,camp_date):
    conn=get_conn(); cur=conn.cursor()
    cur.execute("INSERT INTO camps(title,camp_date) VALUES (?,?)",(title,str(camp_date))); conn.commit(); conn.close()

def fetch_assignments():
    conn=get_conn()
    df=pd.read_sql("""SELECT a.id as assign_id,c.title as camp_title,s.name as staff_name,a.camp_id,a.staff_id
                      FROM assignments a
                      JOIN camps c ON c.id=a.camp_id
                      JOIN staff s ON s.id=a.staff_id
                      ORDER BY c.camp_date DESC""",conn)
    conn.close()
    if df.empty: return pd.DataFrame(columns=["assign_id","camp_title","staff_name","camp_id","staff_id"])
    return df
...
def assign_staff(camp_id,staff_ids):
    ...
    conn.commit(); conn.close(); update_camps_attended()

def update_camps_attended():
    conn=get_conn(); cur=conn.cursor()
    cur.execute("UPDATE staff SET camps_attended=0")
    cur.execute("UPDATE staff SET camps_attended=(SELECT COUNT(*) FROM assignments WHERE staff_id=staff.id)")
    conn.commit(); conn.close()
...
if page=="Dashboard":
    ...
    staff_df=fetch_staff_df(); camps_df=fetch_camps_df()
    ...
    if not staff_df.empty:
        fig=px.pie(staff_df,names="category",title="Staff Distribution by Category",color_discrete_sequence=px.colors.sequential.Plasma)
    ...
    if not pg_df.empty:
        ...
        fig2=px.bar(pg_counts,x="pg_year",y="count",title="PGs by Year",text="count",color="pg_year",color_discrete_sequence=px.colors.qualitative.Dark24)
